...
import { Upload, Video, Languages, Mic2, Sparkles, Check, Download, Share2, X, Copy, MessageCircle, HelpCircle, Rocket, ShieldCheck, AudioLines, RefreshCcw, AlertCircle, PlayCircle, Settings2, Activity } from 'lucide-react';
...
export default function App() {
  ...
  // CHANGED: Defaulting back to 'en' (English) instead of 'pa'
  ...
  const startDubbing = async () => {
    ...
    try {
      ...
      console.error(error);
      const isAuthError = error.message?.toLowerCase().includes('key') || error.message?.includes('401') || error.message?.includes('403');
      setDubbingState({ 
        status: 'error', 
        errorMessage: isAuthError 
          ? "API Key Missing or Invalid on Server. Please check Railway environment variables." 
    ...
    <div className="min-h-screen bg-[#020617] text-slate-200 flex flex-col font-sans selection:bg-indigo-500/30">
        ...
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-5 group cursor-pointer" onClick={hardRefresh}>
            ...
            <div className="flex flex-col">
              <span className="text-2xl font-black text-white tracking-tighter leading-none">DubStudio <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-400">PRO</span></span>
              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.3em] mt-1">AI Synthesis Engine</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
             ...
             <button onClick={() => setShowHelpModal(true)} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 text-xs font-bold text-slate-300 transition-all border border-white/5 group">
                <Settings2 className="w-4 h-4 text-indigo-400 group-hover:rotate-90 transition-transform duration-500" />
        ...
        {!videoUrl ? (
          <div className="max-w-5xl mx-auto text-center space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-1000">
            <div className="inline-flex items-center gap-3 px-6 py-2 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-[11px] font-black uppercase tracking-[0.3em] shadow-2xl">
              {/* FIXED: Replaced index.html with Sparkles icon component */}
          ...
          <div className="grid lg:grid-cols-12 gap-12">
            ...
            <div className="lg:col-span-8 space-y-10">
                ...
                {dubbingState.status === 'error' && (
                   <div className="bg-rose-500/10 border border-rose-500/20 p-12 rounded-[3rem] flex flex-col md:flex-row items-center gap-10 animate-in shake duration-500 shadow-2xl">
                      <div className="w-20 h-20 bg-rose-500 text-white rounded-3xl flex items-center justify-center shadow-2xl shrink-0">
                        <AlertCircle className="w-10 h-10" />
                      </div>
                      <div className="text-center md:text-left">
                        <h4 className="text-2xl font-black text-rose-400 tracking-tighter uppercase">Synthesis Error</h4>
                        <p className="text-sm font-medium text-rose-300 opacity-80 mt-2 leading-relaxed">{dubbingState.errorMessage}</p>
                        <div className="flex flex-wrap gap-4 mt-6 justify-center md:justify-start">
                           <button onClick={() => setShowHelpModal(true)} className="px-6 py-2 bg-rose-500/20 text-rose-400 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-rose-500/30 transition-all border border-rose-500/10">Railway Config</button>
                           <button onClick={hardRefresh} className="px-6 py-2 bg-white/5 text-slate-400 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all">Try Again</button>
                        </div>
                      </div>
                   </div>
                )}
      ...
      <footer className="py-16 text-center border-t border-white/5 bg-slate-950/40">
          <div className="max-w-7xl mx-auto px-6 flex flex-col items-center gap-10">
              ...
              <div className="space-y-4 opacity-30 group hover:opacity-100 transition-opacity duration-700 cursor-default">
                  ...
                  <p className="text-[10px] text-slate-600 font-bold tracking-widest uppercase">Premium Synthesis Suite • 2025 Edition</p>
              </div>
          </div>
      </footer>
      ...
      {showHelpModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-slate-950/95 backdrop-blur-3xl animate-in fade-in duration-300">
            <div className="bg-slate-900 border border-white/10 rounded-[4rem] p-12 w-full max-w-2xl shadow-3xl relative overflow-hidden">
                ...
                <h2 className="text-4xl font-black text-white mb-10 tracking-tighter">Railway Config Guide</h2>
                
                <div className="space-y-8 mb-12">
                    <div className="bg-indigo-500/5 p-8 rounded-[2.5rem] border border-indigo-500/10">
                       <p className="text-xs font-black text-indigo-400 uppercase tracking-[0.3em] mb-4">Required Config</p>
                       <p className="text-slate-300 text-sm leading-relaxed mb-6">
                         Your app is ready for Railway. Ensure you add the <b>API_KEY</b> in the Railway Dashboard.
                       </p>
                       <div className="bg-black/50 p-6 rounded-2xl font-mono text-xs text-indigo-300 border border-white/5">
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                        <div className="p-6 bg-white/5 rounded-3xl space-y-3">
                           <div className="w-10 h-10 bg-indigo-600/20 text-indigo-400 rounded-xl flex items-center justify-center"><Rocket className="w-5 h-5" /></div>
                           <h4 className="font-black text-white">Railway Settings</h4>
                           <p className="text-[11px] text-slate-500 font-medium">Go to 'Variables' and add API_KEY. Railway handles the rest.</p>
                        </div>
                        <div className="p-6 bg-white/5 rounded-3xl space-y-3">
                           <div className="w-10 h-10 bg-indigo-600/20 text-indigo-400 rounded-xl flex items-center justify-center"><ShieldCheck className="w-5 h-5" /></div>
                           <h4 className="font-black text-white">Safe Deployment</h4>
                           <p className="text-[11px] text-slate-500 font-medium">The key is never sent to the browser. It stays on your Railway server.</p>
                        </div>
                    </div>
                </div>
                
                <button onClick={() => setShowHelpModal(false)} className="w-full py-5 bg-white text-black rounded-2xl font-black text-sm tracking-[0.2em] uppercase shadow-2xl hover:scale-[0.98] transition-all">Got it</button>
            </div>
        </div>
      )}
