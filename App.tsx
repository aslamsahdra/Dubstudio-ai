...
export default function App() {
  ...
  const [uiLanguage, setUiLanguage] = useState<UILanguageCode>('en'); // Default is now English
  ...
  useEffect(() => {
    ...
    // Fixed Service Worker Registration for Framed/Proxy Environments
    if ('serviceWorker' in navigator) {
      window.addEventListener('load', () => {
        const swPath = new URL('./sw.js', window.location.href).href;
        // Only register if the origin matches to prevent security errors
        if (new URL(swPath).origin === window.location.origin) {
          navigator.serviceWorker.register(swPath)
            .then(reg => console.log('SW Registered'))
            .catch(err => console.debug('SW Registration ignored (framed environment)'));
        }
      });
    }
  ...
  const startDubbingProcess = () => {
    if (pendingVideoDuration > 120 && (!user || !user.isPremium)) {
      alert(uiLanguage === 'pa' ? "2 ਮਿੰਟ ਤੋਂ ਵੱਡੀ ਵੀਡੀਓ ਲਈ ਪ੍ਰੀਮੀਅਮ ਪਲਾਨ ਚਾਹੀਦਾ ਹੈ।" : "Premium Plan required for videos over 2 minutes.");
      return;
    }
  ...
  return (
    <div className={`min-h-screen bg-[#020617] text-slate-200 flex flex-col font-sans selection:bg-${themeColor}-500/30 overflow-x-hidden transition-colors duration-500`}>
      ...
      {/* PWA INSTALL BANNER */}
      {showInstallBanner && (
        <div className={`bg-gradient-to-r ${themeClasses[themeColor].split(' ').slice(0,2).join(' ')} py-4 px-6 flex items-center justify-between animate-in slide-in-from-top duration-500 z-[200] relative`}>
           <div className="flex items-center gap-4">
              <div className="bg-white/20 p-2.5 rounded-2xl shadow-inner">
                 <Smartphone className="w-5 h-5 text-white" />
              </div>
              <div className="flex flex-col">
                 <p className="text-white font-black text-sm uppercase tracking-tighter">Install DubStudio Master</p>
                 <p className="text-white/70 text-[9px] font-bold uppercase tracking-[0.2em]">{uiLanguage === 'pa' ? 'ਤੇਜ਼ ਅਤੇ ਵਧੀਆ ਅਨੁਭਵ ਲਈ' : 'FOR FASTER & PRO PRODUCTION'}</p>
              </div>
           </div>
           <div className="flex items-center gap-3">
              <button 
                onClick={handleInstallClick} 
                className="bg-white text-black px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-xl"
              >
                 Install Now
              </button>
      ...
      {showAd && (
        <div className="fixed inset-0 z-[1000] bg-black flex flex-col items-center justify-center p-6 sm:p-12 text-center overflow-hidden">
           <div className="absolute top-10 right-10">
              <span className="px-6 py-3 bg-white/10 text-white font-black rounded-2xl text-xl tabular-nums">{adTimer}s</span>
           </div>
           <div className="max-w-2xl space-y-12">
              ...
              <div className="space-y-4">
                <h2 className="text-4xl sm:text-6xl font-black text-white uppercase tracking-tighter">
                  {uiLanguage === 'pa' ? 'ਡੱਬਿੰਗ ਲਈ ਐਡ ਦੇਖੋ' : 'WATCH AD FOR DUBBING'}
                </h2>
                <p className="text-xl text-slate-400 font-bold leading-relaxed">
                  Support Aslam Sahdra Production by watching this short ad to unlock high-fidelity neural dubbing.
                </p>
              </div>
        ...
        {showSettings ? (
          <div className="animate-in fade-in slide-in-from-top-4 duration-500 max-w-4xl mx-auto space-y-12">
            <div className="flex items-center justify-between border-b border-white/10 pb-8">
               ...
               <button onClick={() => setShowSettings(false)} className="p-3 bg-white/5 hover:bg-white/10 rounded-full transition-all"><X className="w-6 h-6" /></button>
            </div>
               ...
               <div className="bg-slate-900/40 border border-white/5 rounded-[3rem] p-10 space-y-8">
                  ...
                  <div className="flex gap-4 pt-4">
                     {(['indigo', 'rose', 'emerald', 'amber', 'purple'] as ThemeColor[]).map(color => (
                        <button key={color} onClick={() => setThemeColor(color)} className={`w-12 h-12 rounded-2xl border-2 ${themeColor === color ? 'border-white scale-110 shadow-lg' : 'border-transparent opacity-60 hover:opacity-100'} bg-${color}-500 transition-all`} />
                     ))}
                  </div>
               </div>
               <div className="bg-slate-900/40 border border-white/5 rounded-[3rem] p-10 space-y-8 flex flex-col justify-center text-center">
                  <div className="w-16 h-16 bg-amber-500/10 rounded-2xl flex items-center justify-center mx-auto mb-4 border border-amber-500/20 shadow-amber-500/10 shadow-xl">
                     <Crown className="w-8 h-8 text-amber-500" />
                  </div>
                  <h3 className="text-xl font-black text-white uppercase tracking-tight">Master Pro Access</h3>
                  <p className="text-xs text-slate-500 font-bold mb-6">Unlimited dubbing, no ads, high-priority processing for pro creators.</p>
                  <button className="w-full py-5 bg-amber-500 text-black rounded-3xl font-black uppercase tracking-[0.2em] text-xs hover:scale-[1.02] transition-all shadow-3xl shadow-amber-500/20">Go Premium</button>
               </div>
            </div>
          </div>
        ) : !videoUrl ? (
          <div className="max-w-5xl mx-auto py-12 text-center space-y-16 animate-in fade-in slide-in-from-bottom-12 duration-1000">
            <div className="space-y-6">
                <div className={`inline-flex items-center gap-3 px-5 py-2.5 bg-${themeColor}-500/10 border border-${themeColor}-500/20 rounded-full text-${themeColor}-400 text-[10px] font-black uppercase tracking-[0.3em] mb-4 shadow-xl`}>
                ...
                <p className="text-slate-500 text-lg sm:text-xl font-bold max-w-2xl mx-auto leading-relaxed uppercase tracking-tight opacity-70">
                    Precision Voice Matching for Adults & Children. Powered by Aslam Sahdra Production.
                </p>
            </div>
            ...
            <div className="max-w-xl mx-auto pt-8">
              <label className="relative group block cursor-pointer transition-all active:scale-95">
                ...
                <div className={`relative flex flex-col items-center gap-8 p-12 sm:p-24 bg-slate-900/60 backdrop-blur-3xl border-2 border-dashed border-slate-800 rounded-[4.5rem] group-hover:border-${themeColor}-500/50 transition-all shadow-3xl`}>
                  <div className={`w-20 h-20 bg-white/5 rounded-[2.5rem] flex items-center justify-center group-hover:bg-${themeColor}-500/10 border border-white/5 shadow-inner`}>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-2 justify-center opacity-40">
                       ...
                       <span className="text-[10px] font-black uppercase tracking-widest">Master Edition v1.5</span>
                    </div>
                  </div>
                </div>
          ...
          <div className="max-w-7xl mx-auto px-6 flex flex-col items-center gap-16">
            ...
            <div className="space-y-10 w-full">
                <div className="flex flex-col items-center gap-8">
                   ...
                   <div className="flex flex-wrap items-center justify-center gap-12 text-center">
                      <div>
                         <p className={`text-xl md:text-3xl text-white font-black tracking-widest uppercase`}>Neural Engine 2025</p>
                         <p className="text-[10px] text-slate-700 font-black uppercase tracking-[0.4em] mt-2">Next-Gen Audio Synthesis</p>
                      </div>
                      <div className="w-px h-12 bg-white/10 hidden md:block" />
                      <div>
                         <p className={`text-xl md:text-3xl text-white font-black tracking-widest uppercase`}>High Fidelity</p>
                         <p className="text-[10px] text-slate-700 font-black uppercase tracking-[0.4em] mt-2">Smart Voice Matching System</p>
                      </div>
                   </div>
                </div>
            </div>
            ...
            <div className="flex flex-col items-center gap-8 pt-20 border-t border-white/5 w-full max-w-5xl">
              <div className="flex flex-wrap items-center justify-center gap-10">
                 <span className="text-[11px] text-slate-700 font-black uppercase tracking-widest">Master Studio Edition v1.5</span>
                 <span className="text-[11px] text-slate-700 font-black uppercase tracking-widest">Global Distribution Rights</span>
                 <span className="text-[11px] text-slate-700 font-black uppercase tracking-widest">Built for Aslam Sahdra</span>
              </div>
              <p className="text-sm text-slate-800 font-black uppercase tracking-[1em] opacity-40">© 2025 ASLAM SAHDRA • ALL RIGHTS RESERVED</p>
            </div>
          </div>
          
          <div className="absolute top-1/2 left-0 -translate-y-1/2 rotate-90 text-[250px] font-black text-white/[0.005] pointer-events-none select-none uppercase tracking-tighter whitespace-nowrap">
            ASLAM SAHDRA
          </div>
          <div className="absolute top-1/2 right-0 -translate-y-1/2 -rotate-90 text-[250px] font-black text-white/[0.005] pointer-events-none select-none uppercase tracking-tighter whitespace-nowrap">
            MASTER EDITION
          </div>
      </footer>
    </div>
